<?php
	...
	if(isset($_POST) & !empty($_POST)){
		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			if(isset($name) && !empty($name)){
				if(($extension == "jpg" || $extension == "jpeg") && $type == "image/jpeg" && $size<=$max_size){
					...
					if(move_uploaded_file($tmp_name, $location.$name)){
						...
						$sql = "INSERT INTO products (name, description, catid, quantity, price, thumb) VALUES ('$prodname', '$description', '$category', '$quantity', '$price', '$location$name')";
			...
			$sql = "INSERT INTO products (name, description, catid, quantity, price) VALUES ('$prodname', '$description', '$category', '$quantity', '$price')";
			$res = mysqli_query($connection, $sql);
			if($res){
				header('location: products.php');
			}else{
		...
		<?php if(isset($smsg)){ ?><div class="alert alert-success" role="alert"> <?php echo $smsg; ?> </div><?php } ?>
			<form method="post" enctype="multipart/form-data">
			  ...
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control" name="productquantity" id="productquantity" placeholder="Quantity">
			  </div>			  
